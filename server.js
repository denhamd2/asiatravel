...
const SHAPE = `{"area":"<neighbourhood, city>","happyHours":[{"name":"","detail":"","when":"","price":""}],"foodDeals":[{"name":"","detail":"","when":"","price":""}],"landmarks":[{"name":"","detail":"","price":""}],"photoSpots":[{"name":"","detail":""}],"familyPicks":[{"name":"","detail":"","price":""}]}`;
...
At most 3 items per category, "detail" max 12 words, empty arrays where nothing solid is found. Only include places genuinely close to that location, drawn from your search results — never invented.`;
  ...
  try {
    const draft = await askClaude(findPrompt);

    // Best-effort verification pass — falls back to the draft if anything goes wrong
    try {
      const checkPrompt = `You are a fact-checker for a travel app. Today is ${now}. Verify this list of places and offers near ${draft.area || "the user's location"} using targeted web searches:

${JSON.stringify(draft)}

Rules:
- Confirm each place exists and is currently operating, and that offers, times, and prices are current. Correct anything wrong.
